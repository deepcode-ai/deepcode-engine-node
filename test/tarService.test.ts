...
describe('TarService', function () {
	it('should extract the registry.tar.gz file', async function () {
		...
		await tarService.extract('/home/user/.deepcode', buffer);

		{
			const stats = await ifs.promises.stat(
				'/home/user/.deepcode/names.json',
			);
		...
		{
			const stats = await ifs.promises.stat(
				'/home/user/.deepcode/-wqkAQr7ILgYeTRozWTEgiUvmSY/config.json',
			);
		...
		{
			const stats = await ifs.promises.stat(
				'/home/user/.deepcode/-wqkAQr7ILgYeTRozWTEgiUvmSY/index.cjs',
			);
